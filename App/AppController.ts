...
export class AppController {
  ...
  /** 崩坏红警叠加层（震屏时闪红） */
  private readonly _CrtAlert: HTMLElement;
  /** CRT 叠加层 */
  readonly CrtOverlay: HTMLElement;
  readonly CrtVignette: HTMLElement;
  /** 可访问性设置（静音 + 减少动画） */
  ...
  constructor(Opts: AppControllerOptions = {}) {
    ...
    // 创建 CRT 崩坏红警叠加层
    this.CrtOverlay = El({ Tag: 'div', Class: 'crt-overlay', Parent: this._MountPoint });
    this.CrtVignette = El({ Tag: 'div', Class: 'crt-vignette', Parent: this._MountPoint });
    // 崩坏红警叠加层（默认透明，触发时闪红）
    this._CrtAlert = El({ Tag: 'div', Class: 'crt-alert', Parent: this._MountPoint });
    // 加载保存的设置项
  ...
  private async _PlayGame(Config: StartConfig, ExistingStore?: IGameStore): Promise<void> {
    ...
    // 创建游戏舞台视图
    const Stage = new GameStageView(Store, Input, AIDirectorInstance, () => this._OpenSettings(), () => this.RequestQuit());
  ...
  private _ResetPageTheme(): void {
    this._MountPoint.style.setProperty('--oasis', COLORS.Oasis);
    this._MountPoint.style.setProperty('--oasis-dim', COLORS.OasisDim);
    this._MountPoint.style.setProperty('--oasis-glow', COLORS.OasisGlow);
  }
  ...
  private _DefaultPlayers(PlayerCount: 2 | 3 | 4): { Name: string; Color: string }[] {
    const Defaults: { Name: string; Color: string }[] = [
      { Name: '玩家1', Color: COLORS.FactionBlue },
      { Name: '玩家2', Color: COLORS.FactionPurple },
      { Name: '玩家3', Color: COLORS.FactionOrange },
      { Name: '玩家4', Color: COLORS.FactionPink },
    ];
  ...
  private _TriggerCollapseFx(): void {
    ...
    this._CrtAlert.classList.add('is-active');
    window.setTimeout(() => {
      this._MountPoint.classList.remove('is-shaking');
      this._CrtAlert.classList.remove('is-active');
    }, 1000);
  }
  ...
  private _LoadSavedSettings(): void {
    try {
      const Crt = localStorage.getItem('second-oasis-crt');
      if (Crt === 'false') {
        this.CrtOverlay.style.display = 'none';
        this.CrtVignette.style.display = 'none';
      }
