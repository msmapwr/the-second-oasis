...
export class NumberPopAnimation extends Animation {
  ...
  constructor(Mount: HTMLElement, X: number, Y: number, Value: number, OnDone?: () => void) {
    ...
    El.style.cssText = `
      ...
      font-weight: bold;
      font-size: 18px;
      color: ${Value >= 0 ? COLORS.Oasis : COLORS.Alert};
      text-shadow: 0 0 6px ${Value >= 0 ? COLORS.Oasis : COLORS.Alert};
      pointer-events: none;
      z-index: 100;
      opacity: 1;
      white-space: nowrap;
  ...
  Update(Dt: number): void {
    ...
    // 向上漂移 40px，同时先放大再淡出
    const Lift = -40 * T;
    const Scale = 1 + 0.3 * Math.sin(Math.PI * T); // 先鼓后缩
    const Opacity = 1 - Math.pow(T, 2); // 后期加速淡出

    this._El.style.transform =
      `translate(${this._StartX}px, ${this._StartY + Lift}px) scale(${Scale})`;
